...
%   rData           - (P x 1 double) Amplitude values of the envelope.
    ...
    % Shift data to ensure positivity, as findpeaks in Octave requires it
    % unless "DoubleSided" is used (which returns both max/min).
    % We want strictly local maxima for upper and local maxima of (-y) for lower.
    shift_val = max(abs(pitchData)) + 100.0;
    
    [peakVals_shifted, peakIdx] = findpeaks(pitchData + shift_val);
    peakVals = peakVals_shifted - shift_val;
    
    if ~isempty(peakIdx)
        peakLocs = tSol(peakIdx);
    else
        peakLocs = [];
    end

    [minVals_shifted, minIdx] = findpeaks(-pitchData + shift_val);
    minVals = minVals_shifted - shift_val;
