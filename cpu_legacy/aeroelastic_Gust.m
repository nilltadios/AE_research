function dxdt= aeroelastic_Gust(t,x,W0,T,gust_type,gammaA,gammaAAA,gammaAAAAA,gammaX,gammaXXX,gammaXXXXX,Us)
dxdt=zeros(size(x));
   
Wratio=0.2;  %frequency ration of the airfoil
phi1=0.165; phi2=0.335; eps1=0.0455; eps2=0.3; mu=100; ah=-1/2; xA=1/4; zetaXi=0; zetaA=0; rA=0.7;  %  

c0=1+1/mu; c1=xA-ah/mu; c2=2/mu*(1-phi1-phi2); 
c3=(1+2*(.5-ah)*(1-phi1-phi2))/mu; 
c4=gammaX*(Wratio/Us)^2+2/mu*(eps1*phi1+eps2*phi2);
c5=(Wratio/Us)^2*gammaXXX;
...
%1-cos gust
if gust_type==2
P=c2*x(4)+c3*x(2)+c4*x(3)+c5*x(3)^3+gammaXXXXX*(Wratio/Us)^2*x(3)^5+c6*x(1)+c7*x(5)+c8*x(6)+c9*x(7)+c10*x(8)-2/mu*((1/2-ah)*x(1,1)+x(3,1))*(phi1*eps1*exp(-eps1*t)+phi2*eps2*exp(-eps2*t))+((W0*sin((pi*t)/(2*T))^2)/50 + (pi*W0*((100*T*(100*pi*cos((pi*t)/T)- 13*T*sin((pi*t)/T)))/(169*T^2 + 10000*pi^2) - (10000*pi*T*exp(-(13*t)/100))/(169*T^2 + 10000*pi^2)))/(200*T) + (pi*W0*((T*(pi*cos((pi*t)/T) - T*sin((pi*t)/T)))/(T^2 + pi^2) - (pi*T*exp(-t))/(T^2 + pi^2)))/(200*T));
...
%constant gust
P=c2*x(4)+c3*x(2)+c4*x(3)+c5*x(3)^3+gammaXXXXX*(Wratio/Us)^2*x(3)^5+c6*x(1)+c7*x(5)+c8*x(6)+c9*x(7)+c10*x(8)-2/mu*((1/2-ah)*x(1,1)+x(3,1))*(phi1*eps1*exp(-eps1*t)+phi2*eps2*exp(-eps2*t))+(W0/50)*(1-.5*exp(-.13*t)-.5*exp(-t));
H=d2*x(2)+d3*x(1)+d4*x(1)^3+gammaAAAAA*(1/Us^2)*x(1)^5+d5*x(4)+d6*x(3)+d7*x(5)+d8*x(6)+d9*x(7)+d10*x(8)+(1+2*ah)/(2*rA^2)*(2/mu*((1/2-ah)*x(1,1)+x(3,1))*(phi1*eps1*exp(-eps1*t)+phi2*eps2*exp(-eps2*t)));
    end
end
